...
#                                                                              #
#                                                         :::      ::::::::    #
#    plot_LEC_specific_steps.py                         :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: daniloceano <danilo.oceano@gmail.com>      +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2024/01/03 23:31:13 by daniloceano       #+#    #+#              #
#    Updated: 2024/09/26 09:10:48 by daniloceano      ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def _plotter(
    ...
    for date, data in daily_means.iterrows():
        ...
        figures_subdirectory = os.path.join(figures_directory, "LEC_specific_steps")
...
def select_specific_steps(all_data):
    # Filter first step for 'incipient', middle step for 'intensification', 
    # middle step for 'mature', and first step for 'decay'

    specific_steps = []

    for system_id, df in all_data.groupby('system_id'):
        incipient_phase = df[df['Phase'] == 'incipient']
        intensification_phase = df[df['Phase'] == 'intensification']
        mature_phase = df[df['Phase'] == 'mature']
        decay_phase = df[df['Phase'] == 'decay']

        if not incipient_phase.empty:
            # First step of incipient phase
            specific_steps.append(incipient_phase.iloc[0])

        if not intensification_phase.empty:
            # Middle step of intensification phase
            middle_idx_intensification = len(intensification_phase) // 2
            specific_steps.append(intensification_phase.iloc[middle_idx_intensification])

        if not mature_phase.empty:
            # Middle step of mature phase
            middle_idx_mature = len(mature_phase) // 2
            specific_steps.append(mature_phase.iloc[middle_idx_mature])

        if not decay_phase.empty:
            # First step of decay phase
            specific_steps.append(decay_phase.iloc[-1])

    # Concatenate the selected specific steps
    specific_steps_df = pd.DataFrame(specific_steps)

    # Average across specific steps for all systems
    mean_specific_steps = specific_steps_df.drop(columns='system_id').groupby('Phase').mean()

    # Standard deviation across specific steps for all systems
    std_specific_steps = specific_steps_df.drop(columns='system_id').groupby('Phase').std()
    
    return mean_specific_steps, std_specific_steps
...
if __name__ == "__main__":
    ...
    # Select specific steps for each system
    periods_df, std_periods_df = select_specific_steps(all_data)
