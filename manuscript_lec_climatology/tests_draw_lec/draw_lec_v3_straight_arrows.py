...
#                                                                              #
#                                                         :::      ::::::::    #
#    draw_lec_v3_straight_arrows.py                     :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: daniloceano <danilo.oceano@gmail.com>      +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2024/01/03 23:31:13 by daniloceano       #+#    #+#              #
#    Updated: 2025/03/24 09:13:17 by daniloceano      ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
def plot_arrow(ax, start, end, term_value, normalized_value, term, phase):
    ...
    size = 5 * normalized_value.loc[phase]
    if size < 1:
        size = 1
...
def plot_signal(ax, positions, phase, data, normalized_data, term):
    ...
    # Definir o tamanho da seta
    normalized_value = normalized_data.loc[phase][term]
...
def plot_term_arrows(ax, size, term, data, normalized_data, positions, phase):
    ...
    # Plot text for residuals
        ...
        end = (positions["∂Az/∂t"][0] + displacement, positions["∂Az/∂t"][1] + size / 2)
        ...
        end = (positions["∂Kz/∂t"][0] + displacement, positions["∂Kz/∂t"][1] + size / 2)

    elif term == "RKe":
        start = (positions["∂Ke/∂t"][0] + displacement, -1)
        end = (positions["∂Ke/∂t"][0] + displacement, positions["∂Ke/∂t"][1] - size / 2)
    ...
    # Plot text for boundaries
        ...
        end = (positions[refered_term][0] - size / 2, positions[refered_term][1] + displacement)
...
def _plotter(phase_means, normalized_data_not_energy):
    ...
    for phase, data in phase_means.iterrows():
        ...
        for term in conversions + residuals + boundaries + budget:
            ...
            if is_balance_term:
                plot_signal(ax, positions, phase, data, normalized_data_not_energy, term)
            else:
                plot_term_arrows(ax, size, term, data, normalized_data_not_energy, positions, phase)
...
def plot_lorenzcycletoolkit(periods_df, figures_directory, normalization_type="log"):
    ...
    figure_path = os.path.join(figures_subdirectory, f"LEC_total_v5_straight_arrows..png")
